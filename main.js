...
# ââ Screen setup âââââââââââââââââââââââââââââââââââââââââââââ
GINIT 780, 420
GCLEAR "#0d0d0d"

# ââ Helper: draw the full UI ââââââââââââââââââââââââââââââââââ
...
# ââ Helper: draw one playlist row
...
# ââ Load and play a track by index
...
# ââ Button hit-test helpers âââââââââââââââââââââââââââââââââââ
...
WHILE running START
    ...
    # strip path â just filename for display
...
CLASS _wplFS
	...
	METHOD listFiles PARAM path
		RETURN __wplFS.listFiles(path)
	END METHOD
...
let wplenv = {
    ...
    listFiles: async (path) => { // part of new FS class in wpl
        let output = await OS.listfiles(path, true);
        console.log(output);
        if(output[1] == false){
            return null; // error // return just null for programming lang instead of output[2]
        } else {
            return output[0]; // [file1, file2, ...], is converted later to WOPLArray
        }
    },
    ...
    getFileSize: async (path) => {
        let output = await OS.getSize(path);
        if (output[1]) {
            return output[0];        } else {
            return null;
        }
    },
    isKeyDown: (key) => {
...
class File {
    ...
    async size() { return await wplenv.getFileSize(this._name); }
...
class WPLFS {
    ...
    async listFiles(path /*WOPLString*/) { return new WOPLArray((await wplenv.listFiles(path.toString())).map(f => new WOPLString(f))); }
...
class ExprParser {
    ...
    parsePrimary() {
        ...
        const builtins = {
            ...
            STRING: (args) => `new WOPLString(woplStr(${args}))`,
...
class OsCalls{
    ...
    async listfiles(path, asArray = false){
        ...
        if(output[1] == false){
            ...
            return [asArray ? entries : entries.join(" "),true,""];
        }
